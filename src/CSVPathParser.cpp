...
// ---bitmap--- //
BitmapSet* bitmap::bmAlloc(unsigned long length,unsigned int blockNum, 
    unsigned int blockSize, bool quote, char*& fileBegin, int threads){
        bool* bitmapField;
        bool* bitmapRecord;
        bool* Field_IR;
        bool* Field_OR;
        ...
        for (int i = 0; i < blockNum-1; i++){
                ...
                if(i == blockNum-1){
                        ...
                        noquote::parse(i, (length-(unsigned long)i*blockSize)/32, linebegin, lineend, mbitmapField, mbitmapRecord);
                }
                else{
                        bool* mField_IR;
                        bool* mField_OR;
                        ...
                        char* linebegin = (char*)  (fileBegin+(unsigned long)blockSize*i);
                        char* lineend = (char*) (fileBegin + length-1);
                        DFA::getDFA(i, linebegin, lineend, 
                                mField_IR, mField_OR, mField_IQ, mField_OQ, 
                                mRecord_IR, mRecord_OR, mRecord_IQ, mRecord_OQ, 
                                Metrix);
        ...
        if(quote == true){
                ...
                for (int counter = 0; counter < blockNum; counter++){
                        ...
                        if(standMetrix[counter] == 0){
                                ...
                                bitmap::mergebitmap(totalBTMPF, totalBTMPR, fenbtmpF, fenbtmpR, (unsigned long) (length - blockSize*counter));
                        }
...
void bitmap::mergebitmap(bool* totalBTMPF, bool* totalBTMPR, bool* fenbtmpF, bool* fenbtmpR, unsigned int blockSize){
        memcpy(totalBTMPF, fenbtmpF, blockSize*sizeof(bool));
        memcpy(totalBTMPR, fenbtmpR, blockSize*sizeof(bool));
}

// -----noquote----- //
noquote::noquote(bool done){
        Done = done;
}
...
DFA* DFA::getDFA(int block, char*&line_begin, char*&line_end, 
                bool*& field_IR, bool*& field_OR, bool*& field_IQ, bool*& field_OQ, 
                bool*& record_IR, bool*& record_OR, bool*& record_IQ, bool*& record_OQ, 
                int*& Metrix){
        int start[4];
        int result[4];
        int symbol;
        ...
        while(line_begin <= line_end){
                ...
                for(int j = 0; j < 4; j++){
                        if(result[j] != 4){
                                ...
                                if(sig[j] == 0 && result[j] == 3)
                                        sig[j] = 1;
                                if(sig[j] == 1 && result[j] == 2)
                                        sig[j] = 0;
                                if(sig[j] == 0){
                                        if(sig_sym == 44){
                                                if(j == 0)
                ...
                ++field_OR;++field_IR;++field_OQ;++field_IQ;
                ++record_OR;++record_IR;++record_OQ;++record_IQ;
                ++line_begin;
        }
        for(int k = 0; k < 4; k++){
